...
scripts.compile = function(args) {
	const container = args.argv[0];
	const tags = args.argv[1];
	
	validateContainer(container);
	
	exec(`rm -rf /tmp/test`);
	exec(`cp -R /sv/containers/${container} /tmp/test`);
	exec(`docker run -it -e PROJECT_ID=sv-shared -e REPO_NAME=${container} -e BRANCH_NAME=master ${tags ? `-e TAGS=${tags}` : ""} -v /tmp/test:/repo -v /var/run/docker.sock:/var/run/docker.sock gcr.io/sv-shared/sv-compile-container:latest`);
}
...
scripts.start = function(args) {
	...
	exec(`helm upgrade ${applicationName} ${appFolder}/ --install --set sv.env=${env} -f /sv/internal/gcrPullSecret.yaml -f /sv/internal/sv.json ${myArgs.join(" ")}`);
}
